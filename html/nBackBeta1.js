...
let expName = 'nBackBeta1';  // from the Builder filename that created this script
...
function experimentInit() {
  ...
  feedback = new visual.Rect ({
    win: psychoJS.window, name: 'feedback', 
...
function trialRoutineBegin(trials) {
  return function () {
    ...
    _trialResp_allKeys = [];
    if ((corrAns === "space")) {
        feedback.fillColor = new util.Color('green');
    } else {
        feedback.fillColor = new util.Color('red');
    }
...
function trialRoutineEachFrame(trials) {
  return function () {
    ...
    // *feedback* updates
    if (((trialResp.keys == 'space')) && feedback.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
    ...
    if (feedback.status === PsychoJS.Status.STARTED && t >= (feedback.tStart + 0.3)) {
    ...
    continueRoutine = false;  // reverts to True if at least one component still running
    for (const thisComponent of trialComponents)
      if ('status' in thisComponent && thisComponent !== feedback && thisComponent.status !== PsychoJS.Status.FINISHED ) {
        continueRoutine = true;
        break;
      }
