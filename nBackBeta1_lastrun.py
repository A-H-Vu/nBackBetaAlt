...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Clifton\\Documents\\psych-proj\\nBackBeta1\\nBackBeta1_lastrun.py',
...
for thisTrials1 in trials1:
    ...
    continueRoutine = True
    # update component parameters for each repeat
    feedback.setPos([loc1, loc2])
    trialSquare.setPos([loc1,loc2])
    trialResp.keys = []
    trialResp.rt = []
    _trialResp_allKeys = []
    if corrAns == 'space':
        feedback.fillColor='green'
    else:
        feedback.fillColor='red'
    # keep track of which components have finished
    trialComponents = [trialGrid, feedback, trialSquare, trialFix, trialResp]
    ...
    while continueRoutine:
        ...
        # *feedback* updates
        if feedback.status == NOT_STARTED and trialResp.keys == 'space':
            # keep track of start time/frame for later
        ...
        if feedback.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > feedback.tStartRefresh + 0.3-frameTolerance:
                # keep track of stop time/frame for later
    ...
    trials1.addData('feedback.started', feedback.tStartRefresh)
    trials1.addData('feedback.stopped', feedback.tStopRefresh)
...
for thisTrials2 in trials2:
    ...
    trials2.addData('feedback.started', feedback.tStartRefresh)
    trials2.addData('feedback.stopped', feedback.tStopRefresh)
...
for thisTrials3 in trials3:
    ...
    trials3.addData('feedback.started', feedback.tStartRefresh)
    trials3.addData('feedback.stopped', feedback.tStopRefresh)
